...
for (const mode of [
  ...
  test(`signed release writeback: ${mode}`, async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "attach-release-"));
    ...
    fs.writeFileSync(
      path.join(remote, "apps/cli/package.json"),
      JSON.stringify({ name: "fixture", version: "1.0.0" }),
    );
    const expected = commit(remote);
    git(root, "clone", remote, cwd);
    const content = JSON.stringify({ name: "fixture", version: "1.0.1" });
    fs.writeFileSync(path.join(cwd, "apps/cli/package.json"), content);
    const context = {
      cwd,
      env: { GITHUB_SHA: expected, GITHUB_REPOSITORY: "uinaf/attach", GITHUB_TOKEN: "fixture" },
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (url, options) => {
      ...
      assert.deepEqual(input.branch, {
        repositoryNameWithOwner: "uinaf/attach",
        branchName: "main",
      });
      assert.deepEqual(input.fileChanges.additions, [
        { path: "apps/cli/package.json", contents: Buffer.from(content).toString("base64") },
      ]);
      ...
      fs.writeFileSync(path.join(remote, "apps/cli/package.json"), content);
    ...
    try {
      if (mode === "normal" || mode === "advanced-after") {
        ...
        assert.equal(fs.readFileSync(path.join(cwd, "apps/cli/package.json"), "utf8"), content);
        ...
        if (mode === "advanced-before")
          assert.match(
            fs.readFileSync(path.join(remote, "apps/cli/package.json"), "utf8"),
            /newDependency/,
          );
...
test("release prepares npm before signed writeback, then publishes GitHub", () => {
  ...
  assert.deepEqual(config.plugins[2], ["@semantic-release/npm", { pkgRoot: "apps/cli" }]);
  assert.equal(config.plugins[3], "./apps/cli/scripts/release-commit.ts");
});
