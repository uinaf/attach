...
export async function prepare(_config: unknown, context: Context): Promise<void> {
  ...
  assert.equal(repository, "uinaf/attach");
  ...
  if (git("diff", "--name-only", expected, "--", ".", ":(exclude)apps/cli/package.json"))
    throw new Error("release preparation changed files other than apps/cli/package.json");
  const before = record(JSON.parse(git("show", `${expected}:apps/cli/package.json`)));
  const content = fs.readFileSync(path.join(cwd, "apps/cli/package.json"));
    ...
    body: JSON.stringify({
      ...
      variables: {
        input: {
          ...
          fileChanges: {
            additions: [{ path: "apps/cli/package.json", contents: content.toString("base64") }],
          },
        },
      },
    }),
  ...
  if (git("diff", "--name-only", expected, committed, "--", ".", ":(exclude)apps/cli/package.json"))
    throw new Error("release writeback changed source outside apps/cli/package.json");
  if (git("show", `${committed}:apps/cli/package.json`) !== content.toString("utf8").trim())
