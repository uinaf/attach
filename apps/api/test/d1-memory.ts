...
export function openMemoryD1(): D1Database {
  ...
  const prepare = (sql: string): D1PreparedStatement => {
    ...
    const stmt: D1PreparedStatement = {
      bind(...values: unknown[]) {
        bounds = values.map((v): SQLInputValue => {
      ...
      async first<T = unknown>(colName?: string) {
        const row = sqlite.prepare(sql).get(...(bounds as SQLInputValue[])) as
      ...
      async run() {
        const info = sqlite.prepare(sql).run(...(bounds as SQLInputValue[]));
        return {
          success: true,
          meta: {
      ...
      async all<T = unknown>() {
        const results = sqlite.prepare(sql).all(...(bounds as SQLInputValue[])) as T[];
        return {
          success: true,
          meta: {
            changes: 0,
